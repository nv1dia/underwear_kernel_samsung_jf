...
make VARIANT_DEFCONFIG=jf_att_defconfig SELINUX_DEFCONFIG=jfselinux_defconfig SELINUX_LOG_DEFCONFIG=jfselinux_log_defconfig underwear_jf_defconfig
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	rm ramdisk.gz
	rm zImage
	rm ../Underwear-kernel-jflteatt-loki*.zip
	zip -r ../Underwear-kernel-jflteatt-loki-$curdate.zip .
	cd $KERNELDIR
else
